...
add_filter( 'bp_get_displayed_user_nav', 'remove_displayed_user_nav' );
...
add_filter( 'bp_get_options_nav_activity-mentions', 'remove_bp_get_options_nav' );
add_filter( 'bp_get_options_nav_public', 'remove_bp_get_options_nav' );


// 管理画面以外で読み込み
if(!is_admin()){
  $directory = str_replace(get_bloginfo('home'), "", get_bloginfo('template_url'));
  // wp と php のfunction.phpを読み込み
  require_once ( '.'.$directory.'/functions/wp.php');
  require_once ( '.'.$directory.'/functions/php.php');
}

/* wordpressカスタマイズ(wp.phpでは動かない分) */
/* カスタム投稿タイプを追加 */
...
function my_front_end_login_fail($username){
  $referrerURL = get_bloginfo('home')."/login/";
  wp_redirect($referrerURL . '?login=failed');
  exit;
}
?>
